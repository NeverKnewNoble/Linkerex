...
const HelpBanner = () => {
  ...
  return (
    <motion.div
      ref={ref}
      className="relative bg-white bg-center bg-no-repeat w-full h-auto py-8 sm:py-10 text-[#18181b] pb-[60px] sm:pb-[100px] pt-[60px] sm:pt-[100px]"
      initial={{ opacity: 0, y: 50 }}
      animate={isInView ? { opacity: 1, y: 0 } : {}}
      transition={{ duration: 1, ease: "easeOut" }}
    >
      {/* Header Section */}
      <div className="flex justify-center text-center mb-6 sm:mb-10">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight max-w-2xl px-4 sm:px-0">
          Help Advance Your Career
        </h1>
      </div>
      ...
      <div className="flex justify-center align-center">
        <motion.div
          className="container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 px-4 sm:px-10"
          initial={{ opacity: 0 }}
          animate={isInView ? { opacity: 1 } : {}}
          transition={{ delay: 0.5, duration: 1 }}
        >
