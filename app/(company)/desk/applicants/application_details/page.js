...
export default function Applicants() {
  ...
  useEffect(() => {
    ...
    const fetchingDetails = async () => {
      try {
        ...
        // Fetch applied job
        const { data: appliedJobs } = await axios.get("/api/applied");
        ...
        // Fetch job details
        const { data: jobList } = await axios.get("/api/jobs");
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const response = await axios.put(`/api/applied/${appliedId}/status`, { updateStatus: status });
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="w-full max-w-[1100px] bg-white/95 dark:bg-gray-900 min-h-screen mx-4 my-6 rounded-xl shadow-2xl p-4 md:p-8"
      >
        {/* Title Section */}
        <h1 className="text-2xl md:text-4xl font-extrabold text-[#18181b] dark:text-white mb-6 text-center">
          Applicant Details
        </h1>

        <section className="space-y-6 md:space-y-8">
          {/* Applicant Information */}
          <Card className="p-4 md:p-6 border border-gray-300 dark:border-gray-700 rounded-xl shadow-md bg-gray-100 dark:bg-gray-800">
            <motion.h2 className="text-xl md:text-2xl font-semibold text-gray-900 dark:text-white mb-4 md:mb-6">
              Applicant Information
            </motion.h2>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
              ...
              <div className="flex-shrink-0">
                <Image
                  ...
                  width={80}
                  height={80}
                  className="w-[80px] h-[80px] md:w-[120px] md:h-[120px] rounded-full border-4 border-white shadow-md object-cover overflow-hidden"
                />
              </div>
          ...
          {/* Job Details */}
          <Card className="p-4 md:p-6 border border-gray-300 dark:border-gray-700 rounded-xl shadow-md bg-gray-100 dark:bg-gray-800">
            <motion.h2 className="text-xl md:text-2xl font-semibold text-gray-900 dark:text-white">
              Job Title:{" "}
          ...
          <Card className="p-4 md:p-6 border border-gray-300 dark:border-gray-700 rounded-xl shadow-md bg-gray-100 dark:bg-gray-800">
            ...
            <div className="space-y-4 md:space-y-6">
              {/* CV Download Section */}
              <div>
                <h4 className="text-base md:text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  CV Download
                </h4>
                {applicantDetails.uploadedFile ? (
                  <a
                    href={`/api/files?file=${applicantDetails.uploadedFile}`} // ✅ Ensure correct file path
                    download
              ...
              <div>
                <h4 className="whitespace-pre-line text-base md:text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">
                  Cover Letter
                </h4>
                <Textarea
              ...
              >
                ...
                <Button 
                  ...
                  className="w-full bg-orange-600 hover:bg-orange-700 text-white py-2 md:py-3 rounded-lg transition-all duration-300"
                >
                  Update Application Status
                </Button>
              </motion.div>
